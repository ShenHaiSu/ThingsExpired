 ...
 * 用户信息
 * @typedef {Object} User
 * @property {number} user_id - 用户ID（主键）
 * @property {string} username - 用户名（唯一）
 * @property {string} email - 邮箱（唯一）
 * @property {number} status - 账户状态（1: 正常）
 * @property {string} created_at - 创建时间，格式: YYYY-MM-DD HH:mm:ss
 */
...
export interface LoginResponse {
  ...
  token: string
  /** Token过期时间，格式: YYYY-MM-DD HH:mm:ss */
  expired: string
}
