...
async function testUserModule(): Promise<string> {
  ...
  const registerResp = await postRequest("/user/register", registerBody);
  ...
  const loginResp = await postRequest<{ token: string }>("/user/login", loginBody);
  console.log("Login Response:", JSON.stringify(loginResp, null, 2));

  // 提取认证 Token
  const authToken = (loginResp as any).data?.token;
...
async function testAuthenticationBranch(authToken: string): Promise<void> {
  ...
  try {
    const authorizedResp = await postRequest("/category/create", categoryBody, authToken);
    console.log("Response:", JSON.stringify(authorizedResp, null, 2));

    const code = (authorizedResp as any).code;
    if (code === 200 || code === 201) {
      log.success("Test Passed: Authorized request correctly accepted");
    } else {
      log.error("Test Failed: Expected success but got error");
    }
  } catch (err: any) {
...
async function testCategoryModule(authToken: string): Promise<void> {
  ...
  const createCategoryResp = await postRequest("/category/create", createCategoryBody, authToken);
  console.log("Create Category Response:", JSON.stringify(createCategoryResp, null, 2));

  // 4. 获取分类列表
  console.log("\n[4] Get Category List Test...");
  const listCategoryResp = await postRequest("/category/list", {}, authToken);
  ...
  const updateCategoryResp = await postRequest("/category/update", updateCategoryBody, authToken);
...
async function testItemModule(authToken: string): Promise<void> {
  ...
  const createItemResp = await postRequest("/item/create", createItemBody, authToken);
  ...
  const listItemResp = await postRequest("/item/list", listItemBody, authToken);
  ...
  const detailItemResp = await postRequest("/item/detail", detailItemBody, authToken);
  ...
  const updateItemResp = await postRequest("/item/update", updateItemBody, authToken);
  ...
  const expiringItemResp = await postRequest("/item/expiring", expiringItemBody, authToken);
  ...
  const deleteItemResp = await postRequest("/item/delete", deleteItemBody, authToken);
...
async function testCleanup(authToken: string): Promise<void> {
  ...
  const deleteCategoryResp = await postRequest("/category/delete", deleteCategoryBody, authToken);
